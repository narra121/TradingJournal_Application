...
import TaskPage from "@/Trades/TradesPage";
...
export default function DashboardPage() {
  ...
  const allTrades: Trade[] = useSelector((state: RootState) => state.TradeData.trades);
  ...
  const handleSelectDate = (date: Date) => {
    ...
    const filteredTrades = allTrades.filter(trade => {
      const tradeDate = parseISO(trade.trade.closeDate);
      return format(tradeDate, 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd');
    });
    setTradesForSelectedDate(filteredTrades);
        ...
        <div className="flex-1 space-y-4 p-8 pt-6">
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            ...
            <TabsContent value="calender">
              <CalendarView data={allTrades} onSelectDate={handleSelectDate} />
              <DailyTradesDialog
