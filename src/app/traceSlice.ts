...
import { ImageType, TradeDetails, Trade, TradesState } from "./types/index";
...
export const addTradeToFirestore = createAsyncThunk(
  ...
  async (tradesToAdd: Trade[], { rejectWithValue, getState }) => {
    try {
      const state = getState() as RootState;
      const userId = state.Auth.user?.uid;
      ...
      tradesToAdd.forEach((trade) => {
        if (!trade || !trade.tradeId) {
          console.warn("Skipping trade without tradeId:", trade);
          return;
        }

        const tradeDocRef = doc(db, `users/${userId}/trades`, trade.tradeId);
        const tradeData = { ...trade };
        delete (tradeData.trade as any).tradeId;
        batch.set(tradeDocRef, tradeData);
      });
